...
C          error return if out of real memory
        ...
        RETURN
        END 
        SUBROUTINE UMS2FA (N, NE, JOB, LVALUE, LINDEX, VALUE,
c       SUBROUTINE UMS2FA (N, NE, JOB, TRANSA, LVALUE, LINDEX, VALUE,
...
C
C  This routine is normally preceded by a call to UMS21I to
C  initialize the default control parameters.  UMS21I need only be
...
C  "INSTALLATION NOTE:" comments in UMS21I
...
C               user.  See UMS21I for details, which sets the defaults.
...
C  Cntl:    A real array of size 10.
C           Must be set by caller on input (not modified).
C           real control arguments, see UMS21I for a description,
...
C## End of user documentation for UMS2FA ###############################
...
C  Version 2.0 only!  Use UMS21I instead.  This subroutine is *not*
...
C  for UMS2CO, UMS2OF, and UMS2R2.  Invalid entries are ignored, and
C  duplicate entries are added together (and the factorization
C  continues).  Warning levels (1..7) will be set later by UMS2ER,
C  above.
C
...
C          -------------------------------------------------------------
           ...
           IF (ERROR .EQ. -1) THEN 
              WRITE (IO, 1) 'UMS2FA: N less than one!'
           ELSE IF (ERROR .EQ. -2) THEN 
              WRITE (IO, 1) 'UMS2FA: NE less than one!'
           ELSE IF (ERROR .EQ. -3) THEN 
              WRITE (IO, 1)
     $        'UMS2FA: LINDEX too small!  Must be greater than ', I
           ELSE IF (ERROR .EQ. -4) THEN 
              WRITE (IO, 1)
     $        'UMS2FA: LVALUE too small!  Must be greater than ', I
...
C          -------------------------------------------------------------
     ...
     $        ' invalid entries ignored (out of range 1..N).'
           ELSE IF (ERROR .EQ. 2) THEN 
              WRITE (IO, 1) 'UMS2FA: ', I,' duplicate entries summed.'
           ELSE IF (ERROR .EQ. 4) THEN 
              WRITE (IO, 1)
     $        'UMS2FA: matrix is singular.  Only ', I, ' pivots found.'
...
C          -------------------------------------------------------------
           ...
           ELSE IF (ERROR .EQ. 99) THEN 
              WRITE (IO, 2)
     $        'UMS2FA: invalid entry (out of range 1..N):', I, J, X
           ELSE IF (ERROR .EQ. 98) THEN 
              WRITE (IO, 2)
     $        'UMS2FA: duplicate entry summed:', I, J, X
...
C          -------------------------------------------------------------
           ...
           IF (ERROR .EQ. -1) THEN 
              WRITE (IO, 1) 'UMS2RF: N less than one!'
           ELSE IF (ERROR .EQ. -2) THEN 
              IF (I .LT. 0) THEN 
                 WRITE (IO, 1) 'UMS2RF: NE less than one!'
              ELSE 
                 WRITE (IO, 1)
     $           'UMS2RF: NE too large!  Must be less than ', I
              ENDIF 
           ELSE IF (ERROR .EQ. -3) THEN 
              WRITE (IO, 1)
     $        'UMS2RF: LINDEX too small!  Must be greater than ', I
           ELSE IF (ERROR .EQ. -4) THEN 
              WRITE (IO, 1)
     $        'UMS2RF: LVALUE too small!  Must be greater than ', I
           ELSE IF (ERROR .EQ. -6) THEN 
              WRITE (IO, 1) 'UMS2RF: pivot order from UMS2FA failed!'
           ELSE IF (ERROR .EQ. -7) THEN 
              WRITE (IO, 1)
     $        'UMS2RF: LU factors uncomputed or corrupted!'
...
C          -------------------------------------------------------------
     ...
     $           ' invalid entries ignored (not in prior pattern).'
              ENDIF 
           ELSE IF (ERROR .EQ. 2) THEN 
              WRITE (IO, 1) 'UMS2RF: ', I,' duplicate entries summed.'
           ELSE IF (ERROR .EQ. 4) THEN 
              WRITE (IO, 1) 'UMS2RF: matrix is singular.  Only ', I,
     $        ' pivots found.'
...
C          -------------------------------------------------------------
           ...
           ELSE IF (ERROR .EQ. 99) THEN 
              WRITE (IO, 2)
     $        'UMS2RF: invalid entry (out of range 1..N):', I, J, X
           ELSE IF (ERROR .EQ. 98) THEN 
              WRITE (IO, 2)
     $        'UMS2RF: duplicate entry summed:', I, J, X
           ELSE IF (ERROR .EQ. 97) THEN 
              WRITE (IO, 2)
     $        'UMS2RF: invalid entry (not in pattern of prior factors)',
     $        I, J, X
           ELSE IF (ERROR .EQ. 96) THEN 
              WRITE (IO, 2)
     $        'UMS2RF: invalid entry (below diagonal blocks):', I, J, X
           ELSE IF (ERROR .EQ. 95) THEN 
              WRITE (IO, 2)
     $        'UMS2RF: invalid entry (prior matrix singular):', I, J, X
...
C          -------------------------------------------------------------
           ...
           IF (ERROR .EQ. -7) THEN 
              WRITE (IO, 1)
     $        'UMS2SO: LU factors uncomputed or corrupted!'
...
C          -------------------------------------------------------------
           ...
           ELSE IF (ERROR .EQ. 8) THEN 
              IF (I .EQ. 0) THEN 
                 WRITE (IO, 1)
     $  'UMS2SO: no iterative refinement: original matrix not preserved'
              ELSE 
                 WRITE (IO, 1)
     $  'UMS2SO: no iterative refinement: only for Ax=b or A''x=b'
              ENDIF 
...
90         CONTINUE 
        ...
        RETURN
        END 
        SUBROUTINE UMS2RF (N, NE, JOB, LVALUE, LINDEX, VALUE,
c       SUBROUTINE UMS2RF (N, NE, JOB, TRANSA, LVALUE, LINDEX, VALUE,
     $          INDEX, KEEP, CNTL, ICNTL, INFO, RINFO)
        INTEGER N, NE, JOB, LVALUE, LINDEX, INDEX (LINDEX), KEEP (20),
     $          ICNTL (20), INFO (40)
        REAL
...
40      CONTINUE 
        RETURN
        END 
        SUBROUTINE UMS2SO (N, JOB, LVALUE, LINDEX, VALUE,
c       SUBROUTINE UMS2SO (N, JOB, TRANSC, LVALUE, LINDEX, VALUE,
