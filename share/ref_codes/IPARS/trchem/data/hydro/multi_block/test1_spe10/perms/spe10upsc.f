...
c 
      ...
      implicit none
c
      integer dimx,dimy,dimz,maxxdim,maxydim,maxzdim,ndivx,ndivy,ndivz
     $       ,nblks
      parameter(maxxdim=1,maxydim=60,maxzdim=220,dimx=1,dimy=60
     $         ,dimz=220,nblks=55)
      integer nx(nblks),ny(nblks),nz(nblks),i,j,k,nb,nxx,nyy,nzz,l
     $       ,factor

      real*8 kx(maxxdim,maxydim,maxzdim),ky(maxxdim,maxydim,maxzdim),
     $       kz(maxxdim,maxydim,maxzdim) 
      real*8 xoff(nblks),yoff(nblks),zoff(nblks),KinX(dimx,dimy,dimz),
     $       KinY(dimx,dimy,dimz),KinZ(dimx,dimy,dimz),
     $       dx(nblks),dy(nblks),dz(nblks),perms,x1,y1,z1,x2,y2,z2,dxx,
     $       dyy,dzz
...
c   of subdomains
      do i=1,nblks
        ...
        j=mod(i,ndivx)
        if(j.ne.0) then
          xoff(i) = (j-1)*1.0D0/ndivx
        else
          xoff(i) = (ndivx-1)*1.0D0/ndivx
        endif

        j=i/ndivx
        j=mod(j,ndivy)
        if(j.ne.0) then
          yoff(i) = (j-1)*1.0D0/ndivy
        else
          yoff(i) = (ndivy-1)*1.0D0/ndivy
        endif

        j=i/(ndivx*ndivy)
        j=mod(j,ndivz)
        if(j.ne.0) then
          zoff(i) = (j-1)*1.0D0/ndivz
        else
          zoff(i) = (ndivz-1)*1.0D0/ndivz
        endif
...
c dim of subdomains
      ...
      open(20,file='spe10l85_55bKxx.dat')
      open(21,file='spe10l85_55bKyy.dat')
      open(22,file='spe10l85_55bKzz.dat')
