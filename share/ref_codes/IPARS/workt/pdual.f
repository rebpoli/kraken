...
C-RT
C      CALL MPI_SEND(IB,ML,MPI_INTEGER,NTP,MSGTAG(MTM),
C     & MPI_COMM_WORLD,IERR)
      CALL MPI_ISEND(IB,ML,MPI_INTEGER,NTP,MSGTAG(MTM),
...
C*********************************************************************
     ...
     & ' ENTERING SUBROUTINE IPIDAT',', TAG =',MSGTAG(MTM)
      ...
      CALL MPI_RECV(IB,ML,MPI_INTEGER,MPI_ANY_SOURCE,
...
C-RT
CC      M=MSGTAG(MTM)+NUMBLK*(34-1)+NUMBLK
CC      M=MSGTAG(MTM)+NUMBLK*(34-1)+NUMBLK*2
        M=MSGTAG(MTM)+NUMBLK*NUMBLK*(34-1)+
...
C      INCLUDE 'msjunk.h'
      ...
      INCLUDE 'mpif.h'
      INTEGER IREQ(NSB),ISTAT(MPI_STATUS_SIZE,NSB)
...
CBW
      ...
      CALL MPI_ISEND(BUFIF4(LOFF,NBF),L,MPI_REAL,NP,M,
...
CBW
      ...
      CALL MPI_RECV(BUFIF4(LOFF,NBF),L,MPI_REAL,NP,M,
     & MPI_COMM_WORLD,ISTATR,IERR)
...
C  WAIT FOR SEND COMPLETIONS
      ...
      CALL MPI_WAITALL (IR,IREQ,ISTAT,IERR)
...
CC      M=MSGTAG(MTM)+NUMBLK*(34-1)+NUMBLK
CC      M=MSGTAG(MTM)+NUMBLK*(34-1)+NUMBLK*2
      M=MSGTAG(MTM)+NUMBLK*NUMBLK*(34-1)+NUMBLK*(NUMBLK-1)+NUMBLK
C-RT
CBW
C      M=MSGTAG(MTM)+NUMBLK*(MAXBUFC-1)+NUMBLK
CBW
...
CBW
      ...
      CALL MPI_ISEND(BUFIF8(LOFF,NBF),L,MPI_DOUBLE_PRECISION,NP,M,
     & MPI_COMM_WORLD,IREQ(IR),IERR)
...
CBW
      ...
      CALL MPI_RECV(BUFIF8(LOFF,NBF),L,MPI_DOUBLE_PRECISION,NP,M,
     & MPI_COMM_WORLD,ISTATR,IERR)
