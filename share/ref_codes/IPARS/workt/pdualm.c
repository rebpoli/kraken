// pdualm.c - C routines for Dual Approximatin Interface, multiple processors

// HISTORY:

// JOHN WHEELER      3/30/99   ORIGINAL ALPHA CODE

#include "memory.h"

// ROUTINE DECLARATIONS

FORTSUB pntibuf4_ (PINT4 n_bufif, PINT4 n_bufdim, PINT4 ntbm);
FORTSUB pntibuf8_ (PINT4 n_bufif, PINT4 n_bufdim, PINT4 ntbm);

//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
FORTSUB pntibuf4_ (PINT4 n_bufif, PINT4 n_bufdim, PINT4 ntbm)
{
// *******************************************************************

// Procedure passes REAL*4 interface buffers and their first dimension
// to SRIBUF4 ().  This routine is called only if  is turned on.

// n_bufif = Memory management index for interface buffers (input)

// n_bufdim = Memory management index for first dimension of
//            interface buffers (input)

// nbtm(m) = Number of buffers (1 to ntbm) to be transmitted to physical
//           model m

// *******************************************************************

extern FORTSUB sribuf4_(PADD buf,PADD bufdim,PINT4 ntbm);

sribuf4_ (aryadd[0][*n_bufif] , aryadd[0][*n_bufdim], ntbm);

return;
}

//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
FORTSUB pntibuf8_ (PINT4 n_bufif, PINT4 n_bufdim, PINT4 ntbm)
{
// *******************************************************************

// Procedure passes REAL*8 interface buffers and their first dimension
// to SRIBUF8 ().  This routine is called only if  is turned on.

// n_bufif = Memory management index for interface buffers (input)

// n_bufdim = Memory management index for first dimension of
//            interface buffers (input)

// nbtm(m) = Number of buffers (1 to ntbm) to be transmitted to physical
//           model m

// *******************************************************************

extern FORTSUB sribuf8_(PADD buf,PADD bufdim, PINT4 ntbm);

sribuf8_ (aryadd[0][*n_bufif] , aryadd[0][*n_bufdim], ntbm);

return;
}
