...
! Else if writing to visual output on subsequent time, append
     ...
     &           0,0,0,0,VTIM,NBLK,0,ISBLOCK,0,0,0,0,0,
...
! Error handling
      ...
      ENDIF
      IF ((.NOT.HYPRE_EVFEM).AND.(NUMBLK.GT.1)) THEN
        CALL KILL_IPARS('VISFLAG=11 with NUMBLK>1 needs HYPRE_EVFEM')
      ENDIF
      IF (.FALSE.) THEN
        IF (NUMBLK.GT.1)
...
! Fill connectivity info
      IFACE = 0
      INODE = 0
      CALL CALLWORK(FILL_FACES,[2,N_LFACES,N_GELEI])
...
! Write single zone header
      ...
      ELSE
        ...
        SHARECONNECTIVITY = 0
        SHAREVARFROMZONE = 0
        IERR = TECZNE112('domain',ZONETYPE,
     &           GNUMNODES,GNUMELES,GNUMFACES,
     &           0,0,0,VTIM,1,0,ISBLOCK,0,0,GNUMFACENODES,0,0,
     &           PASSIVEVARLIST,VALUELOCATION,SHAREVARFROMZONE,
     &           SHARECONNECTIVITY)
        IF (TECDBG) WRITE(*,*)'After TECZNE, IERR=',IERR
      ENDIF
...
! Processor 0 writes X,Y,Z coordinates
     ...
     &  WRITE(*,*)'# X'
      ...
      ELSE
        IERR = TECDAT112(N,NODBUF,ISDOUBLE)
        IF (IERR.NE.0) STOP 'Error with TECDAT112'
      ENDIF
      ENDDO
...
! Write cell-centered variable data
     ...
     &    [2,N_I4U,N_VIS_VARS(IVAR,CURRENT_MODEL)])
        ...
        IF (MYPRC.EQ.0) THEN
          IF (IPRC.GT.0) THEN
            CALL MPI_RECV(BUF,IBUF(1),MPI_DOUBLE_PRECISION,IPRC,
     &        IVAR*10000+IPRC,MPI_COMM_WORLD,ISTAT,IERR)
            IF (IERR.NE.0) STOP 'Error with MPI_RECV'
          ENDIF
          IF (.NOT.TECBIN) THEN
            WRITE(TECUNIT,'(1P,4E15.6)')BUF(1:IBUF(1))
          ELSE
            IERR = TECDAT112(IBUF(1),BUF,ISDOUBLE)
            IF (IERR.NE.0) STOP 'Error with TECDAT112'
          ENDIF
          ELECOUNT=ELECOUNT+IBUF(1)
        ELSE
          IF (MYPRC.EQ.IPRC) THEN
            CALL MPI_SEND(BUF,IBUF(1),MPI_DOUBLE_PRECISION,0,
     &        IVAR*10000+IPRC,MPI_COMM_WORLD,IERR)
...
! Write face-based connectivity information
     ...
     &    WRITE(*,*)'# node count per face'
        IF (MYPRC.EQ.0) THEN
          IF (IPRC.GT.0) THEN
            CALL MPI_RECV(NODPERFACE,IBUF(1),MPI_INTEGER,
     &        IPRC,10000+IPRC,MPI_COMM_WORLD,ISTAT,IERR)
        ...
        ELSE
          IF (MYPRC.EQ.IPRC) THEN
            CALL MPI_SEND(NODPERFACE,IBUF(1),MPI_INTEGER,
     &        0,10000+IPRC,MPI_COMM_WORLD,IERR)
     ...
     &    WRITE(*,*)'# face nodes'
        IF (MYPRC.EQ.0) THEN
          IF (IPRC.GT.0) THEN
            CALL MPI_RECV(FACENODES,IBUF(2),MPI_INTEGER,
     &        IPRC,20000+IPRC,MPI_COMM_WORLD,ISTAT,IERR)
        ...
        ELSE
          IF (MYPRC.EQ.IPRC) THEN
            CALL MPI_SEND(FACENODES,IBUF(2),MPI_INTEGER,
     &        0,20000+IPRC,MPI_COMM_WORLD,IERR)
     ...
     &    WRITE(*,*)'# left elements'
        IF (MYPRC.EQ.0) THEN
          IF (IPRC.GT.0) THEN
            CALL MPI_RECV(LEFTRIGHT(:,1),IBUF(1),MPI_INTEGER,
     &        IPRC,30000+IPRC,MPI_COMM_WORLD,ISTAT,IERR)
        ...
        ELSE
          IF (MYPRC.EQ.IPRC) THEN
            CALL MPI_SEND(LEFTRIGHT(:,1),IBUF(1),MPI_INTEGER,
     &        0,30000+IPRC,MPI_COMM_WORLD,IERR)
     ...
     &    WRITE(*,*)'# right elements'
        IF (MYPRC.EQ.0) THEN
          IF (IPRC.GT.0) THEN
            CALL MPI_RECV(LEFTRIGHT(:,2),IBUF(1),MPI_INTEGER,
     &        IPRC,40000+IPRC,MPI_COMM_WORLD,ISTAT,IERR)
        ...
        ELSE
          IF (MYPRC.EQ.IPRC) THEN
            CALL MPI_SEND(LEFTRIGHT(:,2),IBUF(1),MPI_INTEGER,
     &        0,40000+IPRC,MPI_COMM_WORLD,IERR)
        ...
        IF (MYPRC.EQ.0) THEN
        IF (TECBIN)
     &    IERR = TECPOLYFACE112(IBUF(1),NODPERFACE,FACENODES,
...
!      ELSE
!        IERR = TECPOLYFACE112(LNUMFACES,NODPERFACE,FACENODES,
!     &    LEFTRIGHT(:,1),LEFTRIGHT(:,2))
...
! First we order dual faces
      ...
      IF (LFACES(IA,JA,KA,KD).EQ.-1) THEN
        LFACES(IA,JA,KA,KD)=0
      ENDIF
      LFACES(IA,JA,KA,KD)=LFACES(IA,JA,KA,KD)+1

      IF (KD.LT.4) THEN

      LNUMFACES=LNUMFACES+1
      MBLK=MAX(NBLK,JBLOCK(J))
      STRIDE=NSTRIDE**(NLEVA-MBLK)
      IF (KD.EQ.1) THEN                        ! 2D Face
        LNUMFACENODES=LNUMFACENODES+4*STRIDE
      ELSEIF (ND.EQ.3) THEN                    ! 2D Face
        LNUMFACENODES=LNUMFACENODES+4*STRIDE
      ELSE                                     ! 1D Face
...
! First we order dual faces
      ...
      IF (NBLK.EQ.MBLK) THEN
        IG=IA+IOFF
        JG=JA+JOFF
        KG=KA+KOFF
        CALL FILL_FACE_NODES(MBLK,IG,JG,KG,KD)
      ELSE
        IG=IJKT(1,J)
        JG=IJKT(2,J)
        KG=IJKT(3,J)
        KD2=KD+3
        CALL FILL_FACE_NODES(MBLK,IG,JG,KG,KD2)
      ENDIF

      IF (KD.EQ.2) THEN
        LEFTRIGHT(IFACE,1)=GELEI_EV(J,1,1)
        LEFTRIGHT(IFACE,2)=GELEI(IA,JA,KA)
      ELSE
        LEFTRIGHT(IFACE,1)=GELEI(IA,JA,KA)
        LEFTRIGHT(IFACE,2)=GELEI_EV(J,1,1)
      ENDIF
!          IF ((LEFTRIGHT(IFACE,1).EQ.0).AND.(LEFTRIGHT(IFACE,2).EQ.0))
!     &      WRITE(*,*)'FILL_FACES Error: MYPRC,NBLK,IA,JA,KA,KD=',
!     &      MYPRC,NBLK,IA,JA,KA,KD
