...
)
{
  ...
  else if(processingType_ == "width")
    Info << "Processing the width of the fracture..." << endl;
  ...
  for(int cI=0; cI<totNumLoop; cI++)
  {
    ...
    if(geometry=="flat")
    {
      ...
      // for cell-center width calculation
      build_surface_centers(mesh);
    }
    else if(geometry=="concentricCylinders")
    {
...
void Foam::calcTypes::fieldMap2d::build_surface_centers
(
  const fvMesh& mesh
)
{
  ...
  // get face center coordinates of fracture wall faces
  const vectorField& wallCf = mesh.Cf().boundaryField()[patchID];
  const int numWallFaces = wallCf.size();
  Info << "number of wall faces = " << numWallFaces << endl;
  centersXYonsurface.clear();
  centersXYonsurface.setSize(numWallFaces);
  const int numSingleWallFaces = numWallFaces / 2;
  Info << "number of single wall faces = " << numSingleWallFaces << endl;

  scalar eps=1e-2;
  scalar minInt = min(wallCf.component(intDir)) - eps;
  Info << "minInt = " << minInt << endl;
  scalar maxInt = max(wallCf.component(intDir)) + eps;
  Info << "maxInt = " << maxInt << endl;

  for(int faceI = 0; faceI < numSingleWallFaces; faceI++)
  {
    Info << "face index " << faceI << endl;
    label ind = faceI;

    curMaj = wallCf[faceI].component(majDir);
    Info << "curMaj = " << curMaj << endl;
    curLat = wallCf[faceI].component(latDir);
    Info << "curLat = " << curLat << endl;
    const scalar x = wallCf[faceI].x();
    const scalar y = wallCf[faceI].y();
    const scalar z = wallCf[faceI].z();
    Info << "face center (" << x << ", " << y << ", " << z << ")" << endl;
    
    point searchStart;
    searchStart.component(majDir) = curMaj;
    searchStart.component(latDir) = curLat;
    searchStart.component(intDir) = minInt;
        
    point searchEnd = searchStart;
    searchEnd.component(intDir) = maxInt;
    ...
    centersXYonsurface[hitWallLabel] = hitPoint;
    ...
    if(expNI>1)
    {
      ...
      if ( pHit.hit() )
      {
        centersXYonsurface[secondHitWallLabel] = pHit.hitPoint();
      }
      else
      {
        centersXYonsurface[hitWallLabel] = point::zero;
        centersXYonsurface[secondHitWallLabel] = point::zero;
      }
  ...
  Info << "centers on wall surfaces" << endl;
  forAll(centersXYonsurface, index)
  {
    Info << centersXYonsurface[index] << endl;
  }
...
void Foam::calcTypes::fieldMap2d::write_width
(
  const fvMesh& mesh,
  const Time& runTime
)
{
  fileName current_file_path;
  current_file_path = "postProcessing/dissolCalc" / runTime.timeName() / "width";
  ...
  int count = 0;
  int i = 0;
  List<scalar> widthField(centersXYonsurface.size() / expNI);
  while (i<centersXYonsurface.size() )
  {
    point first = centersXYonsurface[i];
    point second = first;
    ...
    {
      second = centersXYonsurface[i+1];
    }
    
    widthField[i/expNI] = mag( first - second );
    
    i+=expNI;
  }

  scalar minWidth = min(widthField);
  Info << "min width = " << minWidth << endl;
  i = 0;
  if (majDir < latDir) {
    while (i < widthField.size()) {
      widthField[i] -= minWidth;
      apertureMapFile << widthField[i] << "\n";
      i++;    
    }
  }
  else {
    for (int iMaj = 0; iMaj < N_; iMaj++) {
      i = 0;
      for (int iLat = 0; iLat < M_; iLat++) {
	if (iLat == 0) {
	  i = iMaj;
	}
	else {
